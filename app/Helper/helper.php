 ...
 * @return bool|string
 */
function imageKitFileUpload( \Illuminate\Http\UploadedFile $uploadFile ) {
    $fileName = uniqid() . '.' . $uploadFile->getClientOriginalExtension();

    try {
        $uploadFile->move(public_path('uploads/'), $fileName);
        ...
        curl_setopt($ch, CURLOPT_USERPWD, env('IMAGEKIT_PRIVATE_KEY').':');
        curl_setopt($ch, CURLOPT_POSTFIELDS, [
            'file' => new CURLFile( public_path('uploads/'). $fileName ),
            'fileName' => $fileName
        ]);
...
function getSingleMedia($model, $collection = 'profile_image', $skip=true   )
{
    ...
    if (getFileExistsCheck($media)) {
       // return $media->getFullUrl();
         if($collection=="subcategory_image"){
        return "sub/".$media->file_name;
       }
        return "uploads/".$media->id.'/'.$media->file_name;
    }else{
