...
class Service extends Model implements  HasMedia
{
    ...
    protected $table = 'jv_services';
    
    protected $fillable = [
        'name','namefr', 'image', 'category_id', 'provider_id', 'type', 'duration' ,'description','descriptionfr', 'is_featured', 'status' , 'price','subcategory_id'
    ];
     ...
     * @return array
     */
    public static function getServices($provider_id=null)
    {
        $where_clause = " WHERE 1 AND s.deleted_at IS NULL ";

        if( !is_null($provider_id) ) {
            $where_clause .= " AND s.provider_id = $provider_id ";
        }

        $sql = "SELECT 
            s.*,
            c.name AS category_name,
            s_c.name AS sub_category_name
        FROM jv_services AS s
        LEFT JOIN jv_service_categories AS c ON c.id = s.category_id
        LEFT JOIN jv_service_subcategories AS s_c ON s_c.id = s.subcategory_id
        $where_clause
        ORDER BY s.id DESC";
